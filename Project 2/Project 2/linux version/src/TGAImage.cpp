...
bool TGAImage::compareFiles(const char* o, const char* c) {
	ifstream output(o, ios_base::binary | ios_base::ate), 
		correct(c, ios_base::binary | ios_base::ate);

	if (!output.is_open() || !correct.is_open()) {
		throw;// "Error opening file.";
	}

	//	Get total bytes with tellg
	auto fileSize = output.tellg();
	if (fileSize != correct.tellg()) {
		return false;
	}

	output.clear(); //Clears any fail bits
	correct.clear();
	//	Seek to beginning of file
	output.seekg(0, ios_base::beg);
	correct.seekg(0, ios_base::beg);


	//Load the actual pixels
	for (unsigned int i = 0; i < (int)fileSize - 26; i += 8) {
		char a, b;
		output.read((char*)&a, 1),
		correct.read((char*)&b, 1);
		if (a != b) {
			return false;
		}
	}
